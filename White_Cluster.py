...
#----------------- Calcul du Kmeans avec 7 clusters ----------------

k_means_white = cluster.KMeans(n_clusters=7,random_state=0)
k_means_white.fit(white_wine)
label_white = k_means_white.labels_
...
plt.figure(1)
plt.subplot(211)
plt.title("Kmeans avec 7 clusters")
plt.scatter(quality,label_white)
plt.subplot(212)
plt.hist2d(quality,label_white, bins=[7,7])
plt.show()
...
for k in range(2,11) :
  ...
  silhouette_avg = silhouette_score(white_wine, label_white)
  print(
      "For k = ", k,
    "The average silhouette_score of white vine is :",
    silhouette_avg,
  )
